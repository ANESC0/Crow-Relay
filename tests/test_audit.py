...
# ---------------------------------------------------------------------------
# BUG-1 (AUDIT.md) — Blocage PIN ne se prolonge plus indéfiniment via QR
# ---------------------------------------------------------------------------

class TestPinBlockNotExtendedByQrToken:
    """
    Avant le fix : chaque GET /?token=WRONGPIN appelait _record_failure(ip)
    même si l'IP était déjà bloquée, repoussant blocked_until de 10 minutes
    à chaque requête. Un QR bookmarké ou un scanner maintenait le blocage
    indéfiniment.

    Après le fix : _record_failure est ignoré si l'IP est déjà bloquée.
    """
    ...
    def test_blocked_ip_qr_token_does_not_extend_block(self, client):
        """blocked_until ne doit pas augmenter après le 5e échec."""
        for _ in range(5):
            client.get("/?token=WRONGPIN")

        ip = "127.0.0.1"
        assert crow._is_blocked(ip), "l'IP doit être bloquée après 5 échecs"
        blocked_until_initial = crow._login_attempts[ip]["blocked_until"]

        for _ in range(10):
            client.get("/?token=WRONGPIN")

        blocked_until_after = crow._login_attempts[ip]["blocked_until"]
        assert blocked_until_after == blocked_until_initial, (
            "blocked_until ne doit pas être repoussé par des requêtes QR sur une IP déjà bloquée"
        )

    def test_good_token_while_blocked_still_rejected(self, client):
        """Même le bon token est rejeté si l'IP est bloquée."""
        for _ in range(5):
            client.get("/?token=WRONGPIN")

        client.get("/?token=CORRECTPIN")
        assert crow._is_blocked("127.0.0.1")


# ---------------------------------------------------------------------------
# BUG-2 (AUDIT.md) — CF-Connecting-IP ignoré hors mode tunnel
# ---------------------------------------------------------------------------

class TestCFConnectingIPOnlyInTunnelMode:
    """
    _client_ip() ne doit faire confiance à CF-Connecting-IP qu'en TUNNEL_MODE.
    En mode LAN, n'importe qui peut forger ce header pour usurper une IP.
    """

    def test_cf_header_ignored_in_lan_mode(self, client):
        """En mode LAN, CF-Connecting-IP est ignoré — l'IP réelle est utilisée."""
        crow.TUNNEL_MODE = False
        crow.AUTH_ENABLED = True
        crow.PIN = "secret"

        # 5 échecs depuis 127.0.0.1 (remote_addr du test client)
        for _ in range(5):
            client.get("/?token=WRONG", headers={"CF-Connecting-IP": "1.2.3.4"})

        # L'IP bloquée doit être 127.0.0.1, pas 1.2.3.4
        assert crow._is_blocked("127.0.0.1")
        assert not crow._is_blocked("1.2.3.4")

    def test_cf_header_used_in_tunnel_mode(self, client):
        """En mode tunnel, CF-Connecting-IP est utilisé comme IP client."""
        crow.TUNNEL_MODE = True
        crow.AUTH_ENABLED = True
        crow.PIN = "secret"

        for _ in range(5):
            client.get("/?token=WRONG", headers={"CF-Connecting-IP": "5.6.7.8"})

        assert crow._is_blocked("5.6.7.8")
        assert not crow._is_blocked("127.0.0.1")


# ---------------------------------------------------------------------------
# BUG-3 (AUDIT.md) — /api/activity accessible aux appareils can_send uniquement
# ---------------------------------------------------------------------------

class TestActivityFeedPermissions:
    """
    Avant le fix : /api/activity était accessible à tout appareil approved,
    même can_send=True/can_receive=False — fuite d'info.

    Après le fix : can_send OU can_receive suffit (le flux d'activité confirme
    qu'un upload a été reçu — utile pour un émetteur). Ni l'un ni l'autre → [].
    """
    ...
    def _make_device(self, did, can_send, can_receive):
        crow._devices[did] = {
            "name": did,
            "status": "approved",
            "can_send": can_send,
    ...
    def test_can_receive_only_sees_activity(self, client):
        """can_receive=True → feed accessible."""
        self._make_device("recv-only", can_send=False, can_receive=True)
        client.set_cookie("crow_relay_device", "recv-only")
        r = client.get("/api/activity")
        assert r.status_code == 200
        assert isinstance(r.json, list)

    def test_can_send_only_sees_activity(self, client):
        """can_send=True → feed accessible (confirmation d'upload)."""
        self._make_device("send-only", can_send=True, can_receive=False)
        client.set_cookie("crow_relay_device", "send-only")
        r = client.get("/api/activity")
        assert r.status_code == 200
        assert isinstance(r.json, list)

    def test_no_permissions_blocked_from_activity(self, client):
        """can_send=False + can_receive=False → feed vide."""
        self._make_device("no-perms", can_send=False, can_receive=False)
        client.set_cookie("crow_relay_device", "no-perms")
        r = client.get("/api/activity")
        assert r.status_code == 200
        assert r.json == []

    def test_pending_device_blocked_from_activity(self, client):
        """Appareil pending → feed vide."""
        crow._devices["pending-dev"] = {
            "name": "pending",
            "status": "pending",
            "can_send": False,
            "can_receive": False,
        ...
        client.set_cookie("crow_relay_device", "pending-dev")
        r = client.get("/api/activity")
        assert r.json == []


# ---------------------------------------------------------------------------
# BUG-5 (AUDIT.md) — Limite de 3 entrées pending par IP
# ---------------------------------------------------------------------------

class TestPendingPerIpLimit:
    """
    Avant le fix : un attaquant pouvait créer jusqu'à MAX_DEVICES=500 entrées
    pending en saturant /api/request-access depuis une IP unique (ou avec rotation).
    Après le fix : max 3 pending par IP source → 429 au-delà.
    """
    ...
    def test_three_pending_from_same_ip_allowed(self, client):
        """Les 3 premières demandes depuis la même IP passent."""
        for i in range(3):
            client.set_cookie("crow_relay_device", f"dev-ip-{i}")
            r = client.post("/api/request-access", json={"name": f"Device {i}"})
            assert r.status_code == 200, f"La demande {i+1} doit réussir"

    def test_fourth_pending_from_same_ip_returns_429(self, client):
        """La 4e demande depuis la même IP est bloquée."""
        for i in range(3):
            client.set_cookie("crow_relay_device", f"dev-ip4-{i}")
            client.post("/api/request-access", json={"name": f"Device {i}"})

        client.set_cookie("crow_relay_device", "dev-ip4-3")
        r = client.post("/api/request-access", json={"name": "Device 4"})
        assert r.status_code == 429
        assert "Trop de demandes" in r.json.get("error", "")

    def test_approved_devices_dont_count_toward_limit(self, client):
        """Les appareils déjà approuvés n'entrent pas dans le comptage pending."""
        # 2 appareils pending
        for i in range(2):
            client.set_cookie("crow_relay_device", f"dev-mixed-{i}")
            client.post("/api/request-access", json={"name": f"Pending {i}"})

        # 1 appareil approuvé depuis la même IP
        crow._devices["dev-approved"] = {
            "name": "approved",
            "status": "approved",
            "can_send": True,
            "can_receive": True,
        ...
        # Une 3e demande doit encore passer (seuls 2 pending, pas 3)
        client.set_cookie("crow_relay_device", "dev-mixed-new")
        r = client.post("/api/request-access", json={"name": "New"})
        assert r.status_code == 200


# ---------------------------------------------------------------------------
# Second audit — lan_url masqué en mode tunnel pour non-authentifiés
# ---------------------------------------------------------------------------

class TestLanUrlHiddenInTunnelMode:
    """
    lan_url doit être masquée aux non-authentifiés en mode tunnel,
    par symétrie avec tunnel_url. En mode LAN, c'est sans importance
    (l'IP est déjà visible sur le réseau local).
    """
    ...
    def setup_method(self):
        ...
        crow.PIN = "secret"
        crow.TUNNEL_MODE = True
        crow.TUNNEL_URL = "https://abc.trycloudflare.com"
        crow.APPROVAL_ENABLED = False

    def test_lan_url_hidden_from_unauthenticated_in_tunnel(self, client):
        """Sans PIN, lan_url est None en mode tunnel."""
        r = client.get("/api/network-info")
        assert r.status_code == 200
        assert r.json["lan_url"] is None

    def test_lan_url_visible_after_auth_in_tunnel(self, client):
        """Après auth PIN, lan_url est retournée en mode tunnel."""
        client.post("/login", data={"pin": "secret"})
        r = client.get("/api/network-info")
        assert r.json["lan_url"] is not None

    def test_lan_url_always_visible_in_lan_mode(self, client):
        """En mode LAN (sans tunnel), lan_url est visible même sans auth."""
        crow.TUNNEL_MODE = False
        r = client.get("/api/network-info")
        assert r.json["lan_url"] is not None
