...
| EF-6  | Un client avec la permission d'envoi peut supprimer un fichier de l'hôte.      | Moyenne  |
...
| SEC-3  | Le QR code embarque le code PIN dans l'URL ; le PIN est retiré de l'URL après connexion. | Fait     |
...
| GET     | `/api/activity`                | Journal d'activité récent (envois/suppressions). |
...
| GET     | `/admin/boot/<token>`          | Connexion admin automatique (token usage unique, local uniquement). |
| POST    | `/admin/logout`                | Déconnexion du panneau admin.                 |
...
| `--max-mb`           | option CLI   | illimité     | Taille maximale d'un envoi (Mo).        |
| `--cert`             | option CLI   | (auto)       | Chemin vers un certificat TLS personnalisé. |
| `--key`              | option CLI   | (auto)       | Chemin vers la clé privée TLS correspondante. |
| `--no-open`          | option CLI   | désactivé    | Ne pas ouvrir le navigateur au démarrage. |
| `--no-qr`            | option CLI   | désactivé    | Ne pas afficher le QR code dans le terminal. |
