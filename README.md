...
```bash
# Windows
scripts\crow-relay.bat

# macOS / Linux
bash scripts/crow-relay.sh
```
...
python3 app.py --clear-devices    # vider la liste des appareils autorisés au démarrage
...
| `CROW_RELAY_DEVICES_FILE` | Chemin du fichier de registre des appareils | `<dossier app>/devices.json` |
...
1. L'appareil se connecte → écran **"En attente d'autorisation"**, il peut se donner un nom.
2. L'hôte ouvre le **panneau d'autorisation** (s'ouvre automatiquement dans le navigateur).
3. L'hôte accorde le droit d'**envoyer** et/ou de **recevoir** pour chaque appareil.
